export function calculateItem(item) {
  ...
  return {
    ...
    configuredFreeWaterBottles: (quantity / bottlesPerCrate) * waterPerCrate,
    free250mlBottles: free250Enabled ? (quantity / bottlesPerCrate) * free250PerCrate : 0
  }
}
...
export function calculateShopOrder(order) {
  ...
  const summary = items.reduce(
    (summary, item) => {
      ...
      summary.manualFreeWaterBottles += (itemSummary.orderedQuantity / itemSummary.bottlesPerCrate) * manualWaterPerCrate
      return summary
    },
    {
      orderedQuantity: 0,
  ...
  summary.manualFreeWaterBottles = Math.round(summary.manualFreeWaterBottles)
  summary.configuredFreeWaterBottles = Math.round(summary.configuredFreeWaterBottles)
...
export function calculateRouteSummary(route) {
  ...
  for (const order of route.shop_orders || []) {
    ...
    for (const item of order.order_items || []) {
      ...
      const itemSummary = calculateItem(item)
      const itemManualFreeWater = (itemSummary.orderedQuantity / itemSummary.bottlesPerCrate) * Number(order.free_water_per_crate || 0)
