...
export async function getSettingsData() {
  const { supabase, envMissing } = await requireServerSupabase()
  if (envMissing) return { envMissing: true, categories: [], products: [], users: [], appSettings: [], schemes: [] }
  ...
  const [
    ...
    supabase.from('scheme_configurations').select('*, bottle_categories(*)').order('scheme_name', { ascending: true })
  ])
